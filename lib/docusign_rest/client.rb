  ...
  class Client
    ...
    def initialize(options={})
      ...
      if self.auth_method == :oauth
        raise ArgumentError.new("A session record needs to be provided when using oauth") if self.session.blank?
        raise ArgumentError.new("The RSA private key file needs to be specified for oauth") if self.rsa_key_file.blank?
        raise ArgumentError.new('Account ID cannot be empty')  if self.account_id.blank?
        raise ArgumentError.new('Default User ID cannot be empty')  if self.default_user_id.blank?
    ...
    def request_jwt_user_token(expires_in = 3600)
      raise ArgumentError.new('account_id cannot be empty')  if self.account_id.blank?
      raise ArgumentError.new('default_user_id cannot be empty')  if default_user_id.blank?
      raise ArgumentError.new('rsa_key_file cannot be empty')  if self.rsa_key_file.blank?
    ...
    def get_signers(signers, options={})
      ...
      signers.each_with_index do |signer, index|
        ...
        doc_signer[:tabs]             = {
          ...
          declineTabs:          nil,
          dropDown:             get_tabs(signer[:drop_down_tabs], options, index),
          ...
          noteTabs:             get_tabs(signer[:note_tabs], options, index),
    ...
    # headers               - Optional hash of headers to merge into the existing
    #                         required headers for a multipart request.
    # server_templates   -    Array of hashes defining the templates to include, the hash should
    #                         contain two elements:
    #                         template_id - Docusign template ID
    #                         signers - hash defining the details for signers of the template
    #
    # Returns a JSON parsed response body containing the envelope's:
